({
    ...
    upsertLivingExpensesDetails: function (component, event, isNext, isShare) {
        ...
        action.setCallback(this, function(response) {
            var state = response.getState();
            console.log('line 42 Error' , JSON.stringify(response.getError()));
            if (state === "SUCCESS") {
                var ret = response.getReturnValue();
                if(ret=='[]' || $A.util.isUndefinedOrNull(ret)){
                    component.set("v.showError", true);
    ...
    doTotal : function(component, event) {
        ...
        total += $A.util.isUndefined(applicant.Groceries_transport__c) ? 0 : parseFloat( applicant.Groceries_transport__c);
        total += $A.util.isUndefined(applicant.Clothing_and_entertainment__c) ? 0 : parseFloat( applicant.Clothing_and_entertainment__c);
        total += $A.util.isUndefined(applicant.Insurance_and_medical__c) ? 0 : parseFloat( applicant.Insurance_and_medical__c);
        total += $A.util.isUndefined(applicant.Education_and_childcare__c) ? 0 : parseFloat( applicant.Education_and_childcare__c);
