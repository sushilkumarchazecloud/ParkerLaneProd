    ...
    oppExpand : function(component,event,helper){
        ...
        action.setCallback(this, function(response){
            ...
            if (state === "SUCCESS") {
                ...
                //alert(ret); 
                if(ret != null){
                    component.set("v.wrapperData",ret);
                    //Code for seeting up the previous funding requests.
                    var FrqList = component.get("v.wrapperData.FrqList");
                    component.set("v.fundList",FrqList);
                    
                    // Code For StatusColumn(for Colouring) Spliting.
                    var oppStage = component.get("v.wrapperData.Opportunity.StageName");
                    var frList = component.get("v.wrapperData.FrqList");
                    ...
                    var Name = component.get("v.wrapperData.Opportunity.Name")
                    if(Name){
                    ...
                    var actList = [];
                    if(component.get("v.wrapperData.Opportunity.Line_Chart_JSON__c") != null){
                        actList= JSON.parse(component.get("v.wrapperData.Opportunity.Line_Chart_JSON__c"));
                    }
                    let count = 0;
                    if(actList != null ) { 
